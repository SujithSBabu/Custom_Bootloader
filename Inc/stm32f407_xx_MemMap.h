...
#define FLASH_SECTOR2_BASEADDR    0x08008000U /*base addtess of the sector2 of flash memory*/
...
#define MCU_Chip_ID_BASEADDR             0x1FFF7A10
#define MCU_Chip_ID_byte1                (*(volatile uint32_t*)MCU_Chip_ID_BASEADDR)
#define MCU_Chip_ID_byte2                (*(volatile uint32_t*)(MCU_Chip_ID_BASEADDR + 0x00000004))
#define MCU_Chip_ID_byte3                (*(volatile uint32_t*)(MCU_Chip_ID_BASEADDR + 0x00000008))
...
#define CRC_BASE_ADDR        (AHB1PERIPH_BASEADDR + 0x3000U) /* base address of CRC peripheral */
...
typedef struct
{
	__vo uint32_t CRC_DR;
	__vo uint32_t CRC_IDR;
	__vo uint32_t CRC_CR;
}CRC_RegDef_Struct;
...
/*CRC peripheral base address is type casted to structure CRC_RegDef_Struct*/
#define CRC   ((CRC_RegDef_Struct*)CRC_BASE_ADDR)
...
/* Clock enable for CRC peripheral */
#define CRC_CLK_EN()    do{RCC->AHB1ENR |= (1 << 12);} while(0)
